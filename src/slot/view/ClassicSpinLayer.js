var EnumPayTableSpinLayer = require("./EnumPayTableSpinLayer");
 ...
 */
var ClassicSpinLayer = EnumPayTableSpinLayer.extend({
    SPIN_SPEED_STEADY: 1050 * 2 * 1.5,
    /**
    ...
    addPayLine: function () {
        if (this.subjectTmpl.getMaxLineCount() === 1) {
            ...
            this.frontNode.addChild(payLineSprite);
        }
    },
    ...
    initLines: function () {
        ...
        for (var i = 0; i < this.getLineCount(); ++i) {
            ...
            var lineWidth = 3;
            var offsetX = 0;//lineOffsetList[i].x * this.LINE_OFFSET;
            var offsetY = 0;//lineOffsetList[i].y * this.LINE_OFFSET;
    ...
    getDrumModePos: function (localCol) {
        var x = this.panelConfig.spinRegion.x + (localCol) * this.gapWidth + (localCol + 0.5) * this.gridWidth;
        var y = this.panelConfig.spinRegion.y + this.panelConfig.spinRegion.height * 0.5;
        return cc.p(x, y);
    }
});

module.exports = ClassicSpinLayer;